...
public class KirchhoffSpark {
    ...
    public static void main(String[] args) throws Exception {
        ...
        hadoopConf.setLong("FileSplitLength", 2 * 8000L);//文件逻辑切分字节
        hadoopConf.setInt("SplitPerMap", 1);//一个map中多少个键值对

        System.out.println("begin RDD");

        List<Tuple2<Integer, String>> listVelocity = new ArrayList<Tuple2<Integer, String>>();
        for (int i = 0; i < pconf.getOnx(); i++) {
            String result = "";
            for (int j = 0; j < pconf.getOnt(); j++) {
                result += "0,";
            }
            Tuple2<Integer, String> tuple = new Tuple2<Integer, String>(i, result.substring(0, result.lastIndexOf(",")));
            listVelocity.add(tuple);
        }
        JavaPairRDD<Integer, String> rddVelocity = jsc.parallelizePairs(listVelocity, 20).persist(StorageLevel.MEMORY_ONLY());

        for (int i = 0; i < 6; i++) {
            hadoopConf.setInt("index", i);
            if (i == 5) {
                hadoopConf.setInt("count", 7);
            } else {
                hadoopConf.setInt("count", 16);
            }
            JavaPairRDD<Integer, String> pairRDD = jsc.newAPIHadoopFile(otherArgs[0] + "/fcxy.data", MyInputFormat.class, Integer.class, String.class, hadoopConf);
//            JavaPairRDD<Integer, String> splits = pairRDD.flatMapToPair(new SplitKeyValues());

//            List<Tuple2<Integer, String>> lists = splits.collect();
//            for (int t = 0; t < lists.size(); t++) {
//                Tuple2<Integer, String> tuple = lists.get(t);
...
//            List<Tuple2<Integer, String>> lists = output.collect();
...
//            }
            ...
            synchronized (rddVelocity) {
                rddVelocity = rddVelocity.join(output).flatMapToPair(new PairFlatMapFunction<Tuple2<Integer, Tuple2<String, String>>, Integer, String>() {
                    @Override
                    public Iterable<Tuple2<Integer, String>> call(Tuple2<Integer, Tuple2<String, String>> integerTuple2Tuple2) throws Exception {
                        ...
                        String[] str1 = tuple._1().split(",");
                        String[] str2 = tuple._2().split(",");
                        String result = "";
                        for (int i = 0; i < str1.length; i++) {
                            result += (Float.parseFloat(str1[i]) + Float.parseFloat(str2[i])) + ",";
                        }
                        result = result.substring(0, result.lastIndexOf(","));
                        List<Tuple2<Integer, String>> lists = new ArrayList<Tuple2<Integer, String>>();
                        lists.add(new Tuple2<Integer, String>(integerTuple2Tuple2._1(), result));
                        return lists;
                    }
                }).persist(StorageLevel.MEMORY_ONLY());
            }
            rddVelocity.count();
//            List<Tuple2<Integer, String>> lists = rddVelocity.collect();
//            for (int t = 0; t < lists.size(); t++) {
//                Tuple2<Integer, String> tuple = lists.get(t);
//                String[] strs = tuple._2().split(",");
//                System.out.println("key:" + tuple._1() + ",value:" + strs[strs.length / 2] + "," + strs[strs.length - 1]);
//            }
        }
...
//        JavaPairRDD<Integer, String> rdd = output.partitionBy(new ScalaPartitioner(30, pconf.getOnx())).reduceByKey(new ReduceFuction()).sortByKey(true).repartition(1);
        ...
        try {
            ...
            rddVelocity.saveAsNewAPIHadoopFile(path.toString(), Integer.class, String.class, MyOutputFormat.class, hadoopConf);
